...
export default function CardForm({ flashcardId }: { flashcardId: string }) {
  ...
  async function onSubmit(values: CardFormFields) {
    startTransition(async () => {
      try {
        ...
        const { error, message } = await createCardAction(values, flashcardId);
      ...
      <DialogContent className="sm:max-w-[425px] p-6">
        <DialogHeader className="space-y-3">
          ...
          <DialogDescription className="text-gray-500 dark:text-gray-400">
            Create a new card for your flashcard set.
          </DialogDescription>
        </DialogHeader>
